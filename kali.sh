...
wget -qO /tmp/kali.sh https://raw.github.com/fruchttiger00x0/os-scripts/master/kali.sh && bash /tmp/kali.sh -dns -burp -openvas -keyboard de -timezone "Europe/Berlin"
################################################################################
...
#--- Rolling
#rlling_main=deb http://http.kali.org/kali kali-rolling main contrib non-free  # set rolling repo main
#f ! grep -q "$rolling_main" /etc/apt/sources.list /etc/apt/sources.list.d/*; then
 #  echo "deb http://security.kali.org/kali-security kali-rolling/updates main contrib non-free" >> "${file}"
#i
#olling_updates=deb http://http.kali.org/kali kali-rolling main contrib non-free # set rolling repo updates
#f ! grep -q "$rolling_updates" /etc/apt/sources.list /etc/apt/sources.list.d/*; then
